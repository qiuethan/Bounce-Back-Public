...
const addContact = httpsCallable(FIREBASE_FUNCTIONS, "addContact")
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Add Support Contact</Text>
      ...
      <ScrollView contentContainerStyle={styles.form}>
        ...
        <TextInput style={styles.input} value={phone} onChangeText={setPhone} keyboardType="phone-pad" placeholder="(555) 123-4567" placeholderTextColor={COLORS.textMuted} />

        <Text style={styles.label}>Relationship</Text>
        <TouchableOpacity style={styles.input} onPress={() => openDropdown("relationship")}>
          <Text style={{ color: relationship ? COLORS.textDark : COLORS.textMuted }}>{relationship || "Select a relationship..."}</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.input} onPress={() => openDropdown("closeness")}>
          <Text style={{ color: closenessRating ? COLORS.textDark : COLORS.textMuted }}>{closenessRating || "Choose closeness rating..."}</Text>
        </TouchableOpacity>

        <Text style={styles.label}>Contact Frequency</Text>
        <TouchableOpacity style={styles.input} onPress={() => openDropdown("frequency")}>
          <Text style={{ color: contactFrequency ? COLORS.textDark : COLORS.textMuted }}>{contactFrequency || "Select frequency..."}</Text>
        </TouchableOpacity>

        <Text style={styles.label}>Priority</Text>
        <TouchableOpacity style={styles.input} onPress={() => openDropdown("priority")}>
          <Text style={{ color: priorityTag ? COLORS.textDark : COLORS.textMuted }}>{priorityTag || "Select priority..."}</Text>
        </TouchableOpacity>
      ...
      <Modal visible={contactModalVisible} animationType="slide">
        <SafeAreaView style={styles.modalContainer}>
          ...
          />
          <FlatList
            data={contactList.filter((c) => c.name.toLowerCase().includes(searchQuery.toLowerCase())).sort((a, b) => a.name.localeCompare(b.name))}
